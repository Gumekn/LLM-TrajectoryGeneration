...
### 2.2.2 穷举生成模块设计（意图驱动变异）

> **更新说明**：原设计方案使用速度缩放、加速度偏移等参数，已于2026-04-16更新为意图驱动的Jerk/Steering Rate变异方法。

#### 2.2.2.1 变异参数定义（新版）

状态向量：S_t = [v_t, a_t, theta_t, omega_t, x_t, y_t]

| 参数名 | 物理意义 | 颗粒度 | 变异全集 | 物理边界 |
|--------|----------|--------|----------|----------|
| Delta_a | 纵向加速度增量（Jerk效应） | 0.1 m/s² | {-0.5, -0.2, -0.1, 0, 0.1, 0.2} | a_t 属于 [-8.0, 3.0] m/s² |
| Delta_omega | 横摆角速度增量（Steering Rate） | 0.5 度/s | {-1.0, -0.5, 0, 0.5, 1.0} | omega_t 属于 [-20, 20] 度/s |
...
omega_t = clamp(omega_{t-1} + Delta_omega, -20, 20)
...
**基准防发散原则**：
- 若 |a_{t-1}| > 0 且非加减速意图，强制 Delta_a 趋近于 0
- 若 |omega_{t-1}| > 0 且非转向意图，强制 Delta_omega 趋近于 0

**各类意图的有效变异组合**：

| 意图 | Delta_a 可选 | Delta_omega 可选 | 有效组合数 |
|------|-------------|-----------------|-----------|
...
**特殊意图处理**：
- **decelerate_to_stop**：若 v_{t-1} = 0，强制 Delta_a = 0, a_t = 0, v_t = 0
- **emergency_brake**：Delta_omega = 0（锁死方向盘，防侧翻）

#### 2.2.2.4 多意图段组合数

若有 N 个意图段，总变体数 = 每段有效组合数的乘积：
...
N_total = 9 x 9 x 9 = 729

#### 2.2.2.5 与旧方案对比

| 维度 | 旧方案 | 新方案 |
|------|--------|--------|
| 变异参数 | accel_offset, speed_scale, lateral_offset, time_shift, heading_bias | Delta_a, Delta_omega（Jerk/Steering Rate） |
| 参数数量 | 5个参数 | 2个核心参数 |
| 约束方式 | 物理边界过滤 | 意图方向剪枝 |
| 连续性 | 需要平滑插值 | 递推公式自然保证 |
| 组合数 | 141,372（需降采样） | 9-320（意图约束后） |

**新方案优势**：
1. 横纵向严格解耦
2. 物理意义明确（Jerk描述加减速变化，Steering Rate描述转向变化）
3. 意图剪枝大幅减少无效组合
4. 递推公式保证轨迹连续性
